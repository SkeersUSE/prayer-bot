...
Object.keys(users).forEach(id => { if (users[id]?.region) isConfigured(id); });
...
setInterval(() => {
    Object.keys(users).forEach(id => { if (isConfigured(id)) setupScheduler(id); });
}, 12 * 60 * 60 * 1000);

// /start
bot.onText(/\/start/, (msg) => {
    const chatId = msg.chat.id;
    if (isConfigured(chatId)) return bot.sendMessage(chatId, 'Вы настроены! /help');
    
    users[chatId] = { step: 'region', notifications: true, method: 16 };
    saveUsers();
    
    let keyboard = [];
    const names = Object.keys(regions);
    for (let i = 0; i < names.length; i += 2) keyboard.push(names.slice(i, i + 2));
...
// /help
bot.onText(/\/help/, (msg) => {
    bot.sendMessage(msg.chat.id, '/today — сегодня\n/tomorrow — завтра\n/settings — метод\n/region — регион\n/off — выкл уведомления\n/on — вкл уведомления\n/info — о боте');
});

// /info
bot.onText(/\/info/, (msg) => {
    bot.sendMessage(msg.chat.id, 'Официальный бот-расписание Намаза.\n\nОтслеживайте время намазов, получайте напоминания. В настройках можно выбрать регион и метод расчёта.');
});
...
bot.on('callback_query', (query) => {
    ...
    if (data.startsWith('method_')) {
        const key = data.replace('method_', '');
        const user = users[chatId];
        if (!user) return;
        user.method = parseInt(key);
        saveUsers();
        bot.answerCallbackQuery(query.id, { text: 'Метод обновлён!' });
        bot.sendMessage(chatId, 'Метод: ' + methods[key]);
...
bot.on('message', async (msg) => {
    ...
    if (user?.step === 'method') {
        const match = Object.entries(methods).find(([key, name]) => text.startsWith(name));
        if (match) {
            user.method = parseInt(match[0]);
            user.step = 'done';
            saveUsers();
            bot.sendMessage(chatId, 'Настройка завершена! /help', {
...
Object.keys(users).forEach(id => {
    if (isConfigured(id)) setupScheduler(id);
});
